<body style="background-color: #FFA69E;">
<div style="text-align: center; padding: 20px; background-color: #FFA69E; border-radius: 10px;">
    <h1 style="color: #FFFFFF;">All About Vibha!</h1>
    <p style="color: #FFFFFF;"><strong>Learn about me here!</strong></p>
<div style="text-align: center; padding: 20px; background-color: #B7F7F7; border-radius: 10px;">
<style>
  body {
    display: flex;
  }<body style="background-color: #FFA69E;">
  <div style="text-align: center; padding: 20px; background-color: #FFA69E; border-radius: 10px;">
    <h1 style="color: #FFFFFF;">All About Vibha!</h1>
    <p style="color: #FFFFFF;"><strong>Learn about me here!</strong></p>
    <div style="text-align: center; padding: 20px; background-color: #B7F7F7; border-radius: 10px;">
      <style>
        body {
          display: flex;
        }
        #content {
          flex: 1;
          padding: 20px;
          display: flex;
          align-items: center;
        }
        #content img {
          max-width: 100%;
          margin-right: 20px;
        }
      </style>
      <div id="content">
        <img src="https://docs.google.com/drawings/d/e/2PACX-1vQVRanGxsjYlU_dlZvHuuu_76MLIOqTAGBSSeT4AHFB60ZGs80awgc43acBLcFOZuKv8WLTIsFIq1b-/pub?w=960&amp;h=720" alt="Alt Text" style="border-radius: 10px;">
        <div>
          <p style="color: #000000; text-align: left;">
            My name is Vibha, and I am 15 years old. I have a little sister and am a freshman in high school. I enjoy baking, reading, and swimming. In my free time, I like listening to music, watching Netflix, and going to the beach. My favorite color is purple, and my favorite artist is Ariana Grande. My favorite TV shows are Queen's Gambit and Manifest.
          </p>
          <h3 style="font-size: 24px;">Favorite Foods</h3>
          <ul style="list-style-position: outside; padding-left: 20px; margin-top: 0;">
            <li>Burritos</li>
            <li>Brownies</li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</body>


  #content {
    flex: 1;
    padding: 20px;
    display: flex;
    align-items: center;
  }

  #content img {
    max-width: 100%; 
    margin-right: 20px; 
  }
</style>

<div id="content">
  <img src="https://docs.google.com/drawings/d/e/2PACX-1vQVRanGxsjYlU_dlZvHuuu_76MLIOqTAGBSSeT4AHFB60ZGs80awgc43acBLcFOZuKv8WLTIsFIq1b-/pub?w=960&amp;h=720" alt="Alt Text" style="border-radius: 10px;">>
  <div>
    <h1>All About Vibha!</h1>
    <p style="color: #000000; text-align: left;">
        My name is Vibha, and I am 15 years old. I have a little sister and am a freshman in high school. I enjoy baking, reading, and swimming. In my free time, I like listening to music, watching Netflix, and going to the beach. My favorite color is purple, and my favorite artist is Ariana Grande. My favorite TV shows are Queen's Gambit and Manifest.
    </p>
    <h3 style="font-size: 24px;">Favorite Foods</h3>
    <ul style="list-style-position: outside; padding-left: 20px; margin-top: 0;"> <!-- Adjusted margin-top to 0 -->
        <li>Burritos</li>
        <li>Brownies</li>
        <li>Maggi</li>
    </ul>
  </div>
</div>
